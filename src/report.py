...
def build_reports(enriched: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    weekly_revenue = (enriched.groupby(["week", "region"])
        .agg(total_revenue=pd.NamedAgg(column="price", aggfunc="sum"),
    ...
    customer_summary = (enriched.groupby("customer_email")
        .agg(customer_name=pd.NamedAgg(column="customer_name", aggfunc="first"),
             ...
             avg_order=pd.NamedAgg(column="price", aggfunc="mean"),
             order_count=pd.NamedAgg(column="transaction_id", aggfunc="count"))
        .reset_index())
    # TODO: Build category_performance: group by category, columns category/total_revenue/order_count.
    category_performance = (enriched.groupby("category")
        .agg(total_revenue=pd.NamedAgg(column="price", aggfunc="sum"),
             order_count=pd.NamedAgg(column="transaction_id", aggfunc="count"))
        .reset_index())
    # TODO: Build loyalty_analysis: group by loyalty_tier, columns loyalty_tier/avg_spent/customer_count.
    loyalty_analysis = (enriched.groupby("loyalty_tier")
        .agg(avg_spent=pd.NamedAgg(column="price", aggfunc="mean"),
